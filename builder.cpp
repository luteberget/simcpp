...
ReleaseSpec *new_release(size_t trigger) {
  auto r = new ReleaseSpec();
  r->trigger = trigger;
  return r;
}

Plan *new_plan(void) { return new Plan; }
...
double run_plan(InfrastructureSpec *is, Plan *p) { return test_plan(*is, *p); }
void free_plan(Plan *p) { delete p; }

void add_release_resource(ReleaseSpec *r, size_t resource) {
  r->resources.push_back(resource);
}
...
ISObjSpec mkobj(const char *name, int upIdx, double upLength, int downIdx,
                double downLength) {
  ISObjSpec spec;
  spec.name = name;
  spec.n_up = 0;
