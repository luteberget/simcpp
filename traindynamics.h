...
pair<double,double> trainUpdate(const LinearTrainParams& p,
    double v, const LinearTrainStep& s) {
...
LinearTrainStep trainStep(const LinearTrainParams& p, double max_x, double v, 
  ...
  if(v + 1e-10 < r.vmax) {
    ...
    for(auto& ri : r.ahead) {
      ...
      if(r_dx < a_dx+b_dx) {
        double i_dx = (2*p.max_brk*r_dx + r_v*r_v - v*v) / (2*(p.max_acc+p.max_brk));
